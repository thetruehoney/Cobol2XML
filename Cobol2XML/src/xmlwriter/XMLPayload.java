 ...
 */package xmlwriter;
 ...
 public class XMLPayload {
	 ...
	 public void addElements(Cobol c) {
		 ...
		 // List
		 ArrayList<Object> listElement = c.getListElement();
		 if (listElement != null) {
		 ...
		 // Custom
		 String customName  =  c.getCustomName();
		 if (customName != null) {
			 customName = customName + " " + c.getCustomContent();
			 this.addCustom( customName );
			 // Add contents of procedure division
		 } else {
			 // month null
		 }
	 ...
	 void addListElement(ArrayList<Object> listElement) {
		 //  Paragraph element

		 if(listElement != null) {
			 Element cobolname = doc.createElement("list");
			 for (Object s : listElement) {
				 Element item = doc.createElement("item");
				 String st = "" + s.toString();
				 item.appendChild(doc.createTextNode(st));
				 cobolname.appendChild(item);
			 }
			 rootElement.appendChild(cobolname);
		 }
	 }
	 void addCustom(String listElement) {
		 //  Paragraph element
		 String[] s = listElement.split(" ");
		 String string = s[0];
		 Element cobolname = doc.createElement(string);
		 string = s[1];
		 cobolname.appendChild(doc.createTextNode(string));
		 rootElement.appendChild(cobolname);
